...
number*, it yields a routing config that silently sends real traffic to the wrong
tier. So a config minted from judge labels is **untrusted until it clears these
gates**, which measure label quality directly and are judge-agnostic (they behave
the same for a heuristic or an LLM judge). This holds the config to the same
honesty bar calibration already meets (WF-DESIGN-0004: cross-validated lift over
honest baselines), specialized to a judge-sourced label set.
...
def cohens_kappa(pairs: list[tuple[str, str]]) -> float:
    ...
    constant prediction. Never a divide-by-zero or a NaN.
    """
    n = len(pairs)
    if n == 0:
        return 0.0
    ...
    # Chance agreement: sum over labels of P(judge=label) * P(gold=label).
    expected = sum(
...
def confusion_matrix(pairs: list[tuple[str, str]]) -> dict[str, dict[str, int]]:
    ...
    Rows and columns both span the sorted union of every observed label, so every
    cell starts at 0 — including label combinations that never co-occur.
    """
    labels = sorted({label for pair in pairs for label in pair})
...
def cross_validated_accuracy(samples: list[Sample], *, k: int = DEFAULT_CV_FOLDS) -> float:
    ...
    memorizes them. Folds whose training split lacks both labels are skipped (a cut
    needs two). Returns ``0.0`` when no fold is usable.
    ...
    for held_out in range(k):
        test = folds[held_out]
        train = [s for j in range(k) if j != held_out for s in folds[j]]
...
class GateReport:
    ...
    def render(self) -> str:
        ...
        if self.confusion:
            ...
            columns = sorted({col for counts in self.confusion.values() for col in counts})
            lines.append("            " + "  ".join(f"{col:>10}" for col in columns))
            for judge_label in sorted(self.confusion):
                row = self.confusion[judge_label]
                cells = "  ".join(f"{row.get(col, 0):>10}" for col in columns)
                lines.append(f"{judge_label:>10}  {cells}")
        lines.append(f"trust gates: {'PASS' if self.passed else 'REFUSED'}")
...
def evaluate(
    ...
    *,
    ...
    ``gold_pairs`` are ``(judge_label, gold_label)`` for the prompts the judge did
    *not* abstain on (abstentions are excluded from kappa but counted in
    ``gold_abstained``). ``samples`` are the labeled rows the resulting config would
    be fit on.
    """
    ...
    # Two independent if/elif groups. First the gold/kappa gate; then the data-shape
    # gate, where a degenerate set SUPPRESSES the lift message (elif). So a run can
    # yield 0, 1, or 2 failures.
    failures: list[str] = []
    if not gold_pairs:
    ...
    # Field order on the class is contract; kwarg order here is not, so the verdict
    # and its failures lead, then the measured quantities.
    return GateReport(
        passed=not failures,
