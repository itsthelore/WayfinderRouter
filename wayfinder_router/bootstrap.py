...
# The hybrid preset: a keyless local Ollama arm and an OpenAI cloud arm. It best
...
# Cloud arm — OpenAI. The key is read from $OPENAI_API_KEY at request time;
# it is never written here. `export OPENAI_API_KEY=...` before running.
...
# api_key_cmd = "op read op://Private/OpenAI/credential"   # 1Password / Keychain / pass / …
...
_LOCAL_TOML = """\
# wayfinder-router.toml — generated by `wayfinder-router init` (preset: local).
# Delivery stays on this Mac when offline operation is enforced.
...
_APPLE_LOCAL_TOML = """\
# wayfinder-router.toml — generated by `wayfinder-router init --preset apple-local`.
# Selected only after Apple Foundation Models availability is confirmed.
...
PRESETS: dict[str, Preset] = {
    "hybrid": Preset(
        name="hybrid",
        summary="keyless local Ollama → OpenAI cloud",
    ...
    "local": Preset(
        ...
        config_toml=_LOCAL_TOML,
        env_vars=(),
    ),
    "apple-local": Preset(
        name="apple-local",
        summary="Apple Foundation Models on-device local provider",
        config_toml=_APPLE_LOCAL_TOML,
        env_vars=(),
    ),
    "openai": Preset(
        name="openai",
